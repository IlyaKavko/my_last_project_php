...
class Gateway extends \RestApi\Amocrm\AmocrmGateway implements iGateway
{
    ...
    {
        $name = $this->UpdateName($contactModel['name']);
        $Phone = $this->getNumber($contactModel['custom_fields_values'] == null ? 0 : $contactModel['custom_fields_values']);
    ...
    private function getNumber(array $mas): int|null
    {
        $phone_number = 0;
        if ($mas != null || count($mas) > 0) {
            foreach ($mas as $item) {
    ...
    private function createLeads(array $leadModel): iLead
    {
        ...
        $create_time = $leadModel['created_at'];
        $name = $this->UpdateName($NAME);

        return new Lead($id_contact, $name, $id_pipeline, $id_responsible, $create_time);
    }
