...
require_once __DIR__ . '/specifications_product/SpecificationsStruct.php';
...
class Storage extends StoreApp implements iStorage {
	...
	function createCategory(string $title, array $stores, array $specifications): iCategory {
		return $this->getCategories()->createCategory($title, $stores, $specifications);
	}
	...
	function deleteCategory(int $category_id, int $storage_id): bool {
		return $this->getCategoriesToStores()->deleteLink($category_id, $storage_id);
	}

	function restoreCategory( int $category_id, int $storage_id ): bool
	{
		return $this->getCategoriesToStores()->restoreLink($category_id, $storage_id);
	}

	function specificationsGet(int $category_id = null): array|null
	{
		return $this->getSpecifications()->getSpecifications($category_id);
	}

	function specificationsSet( int $category_id, string $title ): bool
	{
		$this->getSpecifications()->setSpecifications($category_id, $title);
		return true;
	}

	function specificationsUpdate( int $specifications_id, string $title ): bool
	{
		$this->getSpecifications()->updateSpecifications($specifications_id, $title);
		return true;
	}

	function specificationsDelete( int $specifications_id): bool
	{
		$this->getSpecifications()->deleteSpecifications($specifications_id);
        return true;
	}

	function specificationsStruct(array $arrStruct): iSpecificationsStruct
    {
        return new SpecificationsStruct($arrStruct);
    }

	function specificationsValueUpdate(array $model): bool
	{
		$this->getSpecificationValue()->updateSpecificationValue($model);
	}

    function specificationsValueCreate(int $specification_id, int $product_id, string $value): bool
    {
        $this->getSpecificationValue()->setSpecificationValue( $specification_id, $product_id, $value);
        return true;
    }

    function specificationsValueGet(int $category_id, int $product_id): iSpecificationsValue
    {
        return $this->getSpecificationValue()->getSpecificationValue($category_id, $product_id);
    }

    function descriptionSet(int $product_id, string $description): bool
    {
        $this->setDescription()->setDescription($product_id, $description);
        return true;
    }

    function descriptionGet(int $product_id): array
    {
        return $this->getDescription()->getDescription($product_id);
    }
    ...
    function setTechnologicalCard(string $title): array {
        return $this->setTechnologicalCardTitle()->setTechnologicalCard($title);
    }

    function getTechnologicalCard(): array {
        return $this->getTechnologicalCards()->getTechnologicalCards();
    }

    function setTechnologicalCardProduct(int $technological_card_id, int $product_id, int $quantity): bool {
        $this->setTechnologicalCardProducts()->setTechnologicalCardProducts($technological_card_id, $product_id, $quantity);
        return true;
    }
