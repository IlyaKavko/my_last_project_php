...
class Categories extends CategoriesSchema implements iCategories {
	...
	public function createCategory(string $title, array $stores_id = [], array $specifications = []): iCategory {
		$this->validStoresId($stores_id);
		$category = $this->insertCategory($title);
		$this->saveStoresLinks($category->getCategoryId(), $stores_id);
		$this->saveSpecifications($category->getCategoryId(), $specifications);
		return $category;
	}
	...
	private function saveSpecifications(int $category_id, array $specifications): void
	{
		if ($specifications !== [])
		{
			foreach ($specifications as $specification)
			{
				$this->app->getSpecifications()->setSpecifications($category_id, $specification);
			}
		}
	}
