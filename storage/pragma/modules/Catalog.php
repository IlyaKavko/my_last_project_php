...
class Catalog extends PragmaStoreDB implements iCatalog {
	...
	function getCategoriesToStoresLinks(): array {
		...
		$sql = "SELECT
                    ...
                    status as deleted
                FROM $links
                WHERE category_id IN (SELECT id FROM $categories WHERE account_id = $this->pragma_account_id)";
		return self::querySql($sql);
	}

	function getCategoriesToStoresActive(int $category_id): array {
		$links = self::getStorageCategoriesToStoresSchema();
		$storesSchema = self::getStorageStoresSchema();
		$sql = "SELECT
                    $links.store_id,
                    $links.category_id,
                    $links.status as deleted,
                    $storesSchema.deleted as store_deleted
                FROM $links

                JOIN $storesSchema ON $storesSchema.id = $links.store_id
                WHERE $links.category_id = $category_id AND $storesSchema.deleted = 1";
		return self::querySql($sql);
	}
