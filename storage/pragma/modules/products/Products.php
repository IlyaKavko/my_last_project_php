...
class Products extends ProductsSchema implements iProducts {
	...
	function deleteProduct(int $product_id): bool {
        ...
        $this->deleteFromDbValuesOfSpecifications($product_id);
        $this->deleteFromDbValuesOfDescription($product_id);
//        $this->deleteFromDbValuesOfTechCard($product_id);
	...
	private function deleteFromDbValuesOfSpecifications(int $product_id): void {
		if (!$this->deleteValuesOfSpecifications($product_id))
			throw new \Exception("Failed to delete values of specifications in $product_id");
	}

	private function deleteFromDbValuesOfDescription(int $product_id): void {
		if (!$this->deleteValuesOfDescriptions($product_id))
			throw new \Exception("Failed to delete values of descriptions in $product_id");
	}

	private function deleteFromDbValuesOfTechCard(int $product_id): void {
		if (!$this->deleteFromDbValuesOfTechCards($product_id))
			throw new \Exception("Failed to delete values of descriptions in $product_id");
	}
