...
use Storage\ProductsCatalog;
use const Storage\EXPORT_STATUS_EXPORTED;
use const Storage\EXPORT_STATUS_LINKED;
use const Storage\EXPORT_STATUS_RESERVED;

require_once __DIR__ . '/../../TestPragmaFactory.php';
require_once __DIR__ . '/../../../../pragma/modules/catalog/ProductsCatalog.php';
...
class ProductsCatalogTest extends \PHPUnit\Framework\TestCase {
    ...
    function testLimit(): void {
	    self::getUniqueProducts(10);
        $this->checkLimit(2, 6, 6);
        $this->checkLimit(null, null, null, 10);
        $this->checkLimit(9, 1, 1);
        $this->checkLimit(999999, 123, 0);
    }

    private function checkLimit(int|null $start, int|null $length, int|null $expect_quantity = null, int $more_then = null): void {
        $filter['limit'] = [];
        if($start) $filter['limit']['start'] = $start;
        if($length) $filter['limit']['length'] = $length;
        $catalog = self::createCatalogFromFilter($filter);
        $products = $catalog->getProducts();
        if(!is_null($expect_quantity))
            $this->assertCount($expect_quantity, $products);
        if(!is_null($more_then))
            $this->assertTrue($more_then <= count($products));
    }
